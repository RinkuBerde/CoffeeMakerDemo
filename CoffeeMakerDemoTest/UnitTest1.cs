...
namespace CoffeeMakerDemoTest
{
    ...
    public class UnitTest1
    {
        ...
        public void OrderACoffee_Should_Return_Received_MessageUsingMock()
        {
            ...
            service.Setup(x => x.CoffeeMaking(It.IsAny<int>(), It.IsAny<int>())).Returns("Your Orders is received");
            // service.Setup(x => x.CoffeeMaking(5,6)).Returns("Your Orders is received");
            StarbuckStore store = new StarbuckStore(service.Object);
            var result = store.OrderCoffee(2, 6);
            Assert.AreEqual("Your Order is received", result);
